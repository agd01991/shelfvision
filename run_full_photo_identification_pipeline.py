...
from src.identification.clustered_gallery_builder import (
    build_clustered_demo_sku_gallery_from_predictions,
)
from src.identification.demo_gallery_builder import (
    build_demo_sku_gallery_from_predictions,
)
from src.identification.matcher import run_sku_matching
from src.identification.metrics import (
    evaluate_with_ground_truth,
    save_identification_metrics,
)
...
from src.reporting.segmentation_identification_report import (
    generate_segmentation_identification_report,
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Полный пайплайн ShelfVision для фото-идентификации с разделением на gallery/query"
    )
    parser.add_argument(
        "--model",
        choices=["yolo", "yolo_seg", "rtdetr", "frcnn"],
        default="yolo",
        help="Модель инференса",
    )
    parser.add_argument("--weights", required=True, help="Файл весов модели")
    parser.add_argument(
        "--images-dir",
        default=None,
        help="Общая папка изображений. Используется, если не заданы отдельные gallery/query папки",
    )
    parser.add_argument(
        "--gallery-images-dir",
        default=None,
        help="Отдельная папка изображений для формирования галереи",
    )
    parser.add_argument(
        "--query-images-dir", default=None, help="Отдельная папка query-изображений"
    )
    parser.add_argument(
        "--out-dir",
        default="D:/1Diplom/shelfvision_results/full_photo_identification",
        help="Папка результатов полного эксперимента",
    )
    parser.add_argument(
        "--gallery-dir",
        default="D:/1Diplom/sku_gallery_full",
        help="Папка создаваемой SKU-галереи",
    )
    parser.add_argument(
        "--gallery-csv",
        default="D:/1Diplom/sku_gallery_full/gallery.csv",
        help="CSV-файл создаваемой SKU-галереи",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Общий лимит изображений из --images-dir перед разделением. 0 означает все",
    )
    parser.add_argument(
        "--gallery-count",
        type=int,
        default=50,
        help="Сколько изображений из --images-dir отправить в gallery split",
    )
    parser.add_argument(
        "--query-count",
        type=int,
        default=0,
        help="Сколько изображений отправить в query split. 0 означает все оставшиеся",
    )
    parser.add_argument(
        "--gallery-limit",
        type=int,
        default=0,
        help="Лимит для явной gallery-папки. 0 означает все",
    )
    parser.add_argument(
        "--query-limit",
        type=int,
        default=0,
        help="Лимит для явной query-папки. 0 означает все",
    )
    parser.add_argument(
        "--shuffle",
        action="store_true",
        help="Перемешать изображения перед разделением на gallery/query",
    )
    parser.add_argument(
        "--seed",
        type=int,
        default=42,
        help="Начальное число для воспроизводимого перемешивания",
    )

    parser.add_argument(
        "--conf", type=float, default=0.25, help="Порог confidence для детектора"
    )
    parser.add_argument(
        "--imgsz", type=int, default=640, help="Размер изображения для инференса"
    )
    parser.add_argument(
        "--device", default=None, help="Устройство инференса, например 0 или cpu"
    )
    parser.add_argument(
        "--bbox-only",
        action="store_true",
        help="Использовать только ограничивающие прямоугольники без масок",
    )

    parser.add_argument(
        "--max-sku", type=int, default=100, help="Максимальное число demo SKU"
    )
    parser.add_argument(
        "--min-score",
        type=float,
        default=0.35,
        help="Минимальная confidence для crop-объекта галереи",
    )
    parser.add_argument(
        "--min-width", type=int, default=20, help="Минимальная ширина crop-объекта"
    )
    parser.add_argument(
        "--min-height", type=int, default=20, help="Минимальная высота crop-объекта"
    )
    parser.add_argument(
        "--padding", type=float, default=0.05, help="Отступ вокруг crop-объекта"
    )
    parser.add_argument(
        "--prefix", default="sku_demo_", help="Префикс для создаваемых demo SKU"
    )
    parser.add_argument(
        "--keep-old-demo",
        action="store_true",
        help="Не удалять старые demo SKU перед сборкой новой галереи",
    )
    parser.add_argument(
        "--no-deduplicate-gallery",
        action="store_true",
        help="Отключить объединение визуально похожих crop-объектов в один SKU",
    )
    parser.add_argument(
        "--dedup-threshold",
        type=float,
        default=0.86,
        help="Порог сходства для жадного объединения crop-объектов в один demo SKU",
    )
    parser.add_argument(
        "--max-refs-per-sku",
        type=int,
        default=3,
        help="Максимум эталонных изображений на demo SKU после объединения",
    )
    ...
    parser.add_argument(
        ...
        default="greedy",
        help="Режим построения demo SKU-галереи",
    )
    parser.add_argument(
        "--cluster-merge-threshold",
        type=float,
        default=0.82,
        help="Кластерная галерея: порог объединения по центроидам/парам",
    )
    parser.add_argument(
        "--cluster-strong-merge-threshold",
        type=float,
        default=0.88,
        help="Кластерная галерея: порог сильного совпадения по одной паре",
    )
    parser.add_argument(
        "--cluster-min-similarity",
        type=float,
        default=0.72,
        help="Кластерная галерея: минимальное сходство внутри объединённого кластера",
    )
    parser.add_argument(
        "--cluster-pair-report-threshold",
        type=float,
        default=0.75,
        help="Кластерная галерея: сохранять пары SKU выше этого сходства",
    )
    parser.add_argument(
        "--cluster-max-candidates",
        type=int,
        default=0,
        help="Кластерная галерея: максимум предварительных кандидатов. 0 означает автоматически",
    )

    parser.add_argument(
        "--threshold", type=float, default=0.65, help="Порог уверенного совпадения SKU"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Список порогов для анализа качества",
    )
    parser.add_argument(
        "--top-k", type=int, default=3, help="Количество ближайших SKU-кандидатов"
    )
    parser.add_argument(
        "--gt-csv", default=None, help="CSV с эталонными SKU, если он есть"
    )
    parser.add_argument(
        "--visualize-limit",
        type=int,
        default=100,
        help="Сколько визуализаций идентификации сохранить",
    )

    parser.add_argument(
        "--resume",
        action="store_true",
        help="Переиспользовать predictions_partial.jsonl при наличии",
    )
    parser.add_argument(
        "--skip-existing",
        action="store_true",
        help="Переиспользовать итоговый predictions.json, если он соответствует текущему split",
    )
    parser.add_argument(
        "--progress-every",
        type=int,
        default=10,
        help="Печатать прогресс каждые N изображений/объектов",
    )
    parser.add_argument(
        "--no-visualize-inference",
        action="store_true",
        help="Не сохранять визуализации инференса для gallery/query",
    )
...
def _split_images(args: argparse.Namespace) -> tuple[List[Path], List[Path]]:
    if args.gallery_images_dir or args.query_images_dir:
        if not args.gallery_images_dir or not args.query_images_dir:
            raise SystemExit(
                "Укажи одновременно --gallery-images-dir и --query-images-dir либо используй только --images-dir"
            )
        gallery = _maybe_shuffle(
            _list_images(args.gallery_images_dir, args.gallery_limit), args
        )
        query = _maybe_shuffle(
            _list_images(args.query_images_dir, args.query_limit), args
        )
        return gallery, query

    if not args.images_dir:
        raise SystemExit(
            "Укажи --images-dir или одновременно --gallery-images-dir и --query-images-dir"
        )
    ...
    if not query:
        raise SystemExit(
            "Query split пустой. Увеличь --limit или уменьши --gallery-count"
        )
    return gallery, query
...
def _save_manifest(
    ...
    pd.DataFrame([asdict(row) for row in rows if row.split == "gallery"]).to_csv(
        gallery_csv, index=False
    )
    pd.DataFrame([asdict(row) for row in rows if row.split == "query"]).to_csv(
        query_csv, index=False
    )
    split_params_json = manifest_dir / "split_params.json"
    split_params_json.write_text(
        json.dumps(
            {
                "split_level": "image",
                "split_method": "shuffled_then_first_gallery_rest_query"
                if args.shuffle
                else "sorted_then_first_gallery_rest_query",
                "shuffle": bool(args.shuffle),
                "seed": int(args.seed),
                "gallery_images_count": len(gallery),
                "query_images_count": len(query),
                "total_images_count": len(gallery) + len(query),
            },
            ensure_ascii=False,
            indent=2,
        ),
    ...
    return {
        "all_images": all_csv,
        "gallery_images": gallery_csv,
        "query_images": query_csv,
        "split_params": split_params_json,
    }


def _validate_split_disjoint(gallery: Sequence[Path], query: Sequence[Path]) -> None:
    gallery_set = {path.resolve() for path in gallery}
    query_set = {path.resolve() for path in query}
    intersection = gallery_set & query_set
    if intersection:
        raise ValueError(
            "Gallery/query split пересекается по изображениям: "
            + ", ".join(str(path) for path in sorted(intersection)[:10])
        )
...
def _run_split_inference(
    split_name: str, images: Sequence[Path], args: argparse.Namespace, out_dir: Path
) -> Path:
    ...
    if (
        args.skip_existing
        and predictions_json.exists()
        and _can_reuse_predictions(predictions_json, images)
    ):
        print(
            f"PHOTO_PROGRESS split={split_name} reused_existing={predictions_json}",
            flush=True,
        )
    ...
    predictions_by_path = {
        path: pred
        for path, pred in predictions_by_path.items()
    ...
    print(
        f"PHOTO_PROGRESS split={split_name} total={total} resume_loaded={processed_before}",
        flush=True,
    )
    with partial_jsonl.open("a", encoding="utf-8") as partial:
        for image_path in images:
            ...
            if not args.no_visualize_inference:
                draw_prediction(
                    pred,
                    output_path=visualized_dir / image_path.name,
                    show_masks=not args.bbox_only,
                )

            done = len(predictions_by_path)
            elapsed = time.perf_counter() - start_time
            rate = (
                max(1e-9, (done - processed_before) / elapsed)
                if done > processed_before
                else 0.0
            )
            ...
            if done == 1 or done % max(1, args.progress_every) == 0 or done == total:
                objects = sum(
                    item.objects_count for item in predictions_by_path.values()
                )
                print(
                    "PHOTO_PROGRESS "
    ...
    ordered = [
        predictions_by_path[str(path)]
        for path in images
        if str(path) in predictions_by_path
    ]
...
def _build_demo_gallery(
    args: argparse.Namespace, gallery_predictions_json: Path, demo_dir: Path
) -> Dict[str, Path]:
...
def _save_full_summary(
    ...
    summary = FullExperimentSummary(
        ...
        extracted_gallery_crops_count=int(
            demo_summary.get("extracted_crops_count", 0) or 0
        ),
        ...
        identification_results_csv=str(
            identification_dir / "identification_results.csv"
        ),
        visualized_dir=str(identification_dir / "visualized"),
        threshold_analysis_csv=str(threshold_outputs.get("threshold_analysis_csv", "")),
        assignment_uncertainty_report_md=str(
            assignment_outputs.get("assignment_uncertainty_report_md", "")
        ),
        segmentation_identification_report_md=str(
            segmentation_identification_outputs.get(
                "segmentation_identification_report_md", ""
            )
        ),
        params={
            "model": args.model,
    ...
    pd.DataFrame([{k: v for k, v in raw.items() if k != "params"}]).to_csv(
...
def main() -> None:
    ...
    env_path = _save_run_environment(out_dir, args)
    print(f"Среда запуска сохранена: {env_path}", flush=True)
    ...
    manifests = _save_manifest(out_dir, gallery_images, query_images, args)
    print(f"Манифест сохранён: {manifests['all_images']}", flush=True)
    print(
        f"Изображений для gallery: {len(gallery_images)} | изображений для query: {len(query_images)}",
        flush=True,
    )
    if args.shuffle:
    ...
    gallery_predictions_json = _run_split_inference(
        "gallery", gallery_images, args, gallery_inference_dir
    )

    print("Шаг 2/5: сборка demo SKU-галереи по gallery-части", flush=True)
    _build_demo_gallery(
        args=args, gallery_predictions_json=gallery_predictions_json, demo_dir=demo_dir
    )

    print("Шаг 3/5: инференс query-части", flush=True)
    query_predictions_json = _run_split_inference(
        "query", query_images, args, query_inference_dir
    )
    ...
    threshold_outputs = save_threshold_analysis(
        results,
        out_dir=reports_dir,
    ...
    for name, path in {
        **threshold_outputs,
        **assignment_outputs,
        **segmentation_identification_outputs,
        **summary_outputs,
        **vkr_outputs,
    }.items():
    ...
    print(
        f"Неоднозначных совпадений: {metrics.get('matched_uncertain', 0)}", flush=True
    )
    print(f"Неопределённых объектов: {metrics.get('unknown', 0)}", flush=True)
    print(
        f"Отчёт по связке сегментации и идентификации: {segmentation_identification_outputs.get('segmentation_identification_report_md')}",
        flush=True,
    )


def _save_run_environment(out_dir: Path, args: argparse.Namespace) -> Path:
    import platform
    import sys
    import subprocess

    def version_or_none(module_name: str) -> str:
        try:
    ...
    payload = {
        ...
        "ultralytics": version_or_none("ultralytics"),
        "opencv": version_or_none("cv2"),
    ...
    try:
        ...
        payload["nvidia_smi"] = f"not_available: {exc}"

    path = out_dir / "00_manifest" / "run_environment.json"

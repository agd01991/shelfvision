...
4. `identification_results.csv` с колонками статусов.
5. `validation_report.md` как подтверждение корректности выходной папки.
6. `run_manifest.json` как паспорт воспроизводимости.
7. `matched_uncertain_report.md` как пример выделения спорных случаев.

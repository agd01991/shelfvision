...
ROOT = Path(__file__).resolve().parents[1]  # корень проекта shelfvision
DATASET_DIR = (ROOT / "data" / "yolo_cache" / "sku110k_small_v1_tiled").resolve()
DATA_YAML = DATASET_DIR / "dataset.yaml"

RUNS_LOCAL = (ROOT / "runs" / "yolo_night").resolve()  # куда пишет Ultralytics
BACKUP_DIR = (
    ROOT / "runs_night_backup"
).resolve()  # куда копируем после каждого эксперимента
BACKUP_DIR.mkdir(parents=True, exist_ok=True)

STATE_PATH = BACKUP_DIR / "state.json"
SUMMARY_PATH = BACKUP_DIR / "summary.csv"

DEVICE = "0"  # "0" для GPU, "cpu" если нужно
SEED = 42

# RTX 3070 Laptop (8GB) часто не тянет batch=8 на imgsz=1024 → безопаснее batch=4
BASE = dict(
    data=str(DATA_YAML),
    imgsz=1024,
    epochs=30,
    ...
    plots=False,
    verbose=False,
    save_period=1,  # сохранять last.pt каждый epoch (страховка)
)

BASE_MODEL = "yolov8s.pt"
BASE_NAME = "BASE_yolov8s_img1024_e30_b4"


EXPERIMENTS = [
    # меняется только model
    dict(name="E01_model_yolov8n", model="yolov8n.pt", override={}),
    dict(name="E02_model_yolov8m", model="yolov8m.pt", override={}),
    # # меняется только imgsz
    dict(name="E03_imgsz_640", model=BASE_MODEL, override={"imgsz": 640}),
    dict(name="E04_imgsz_1280", model=BASE_MODEL, override={"imgsz": 1280}),
    # # меняется только lr0
    dict(name="E05_lr0_0p005", model=BASE_MODEL, override={"lr0": 0.005}),
    # # меняется только weight_decay
    dict(name="E06_wd_0p001", model=BASE_MODEL, override={"weight_decay": 0.001}),
    # # меняется только optimizer
    dict(name="E07_optimizer_AdamW", model=BASE_MODEL, override={"optimizer": "AdamW"}),
    # # меняется только mosaic
    dict(name="E08_mosaic_0", model=BASE_MODEL, override={"mosaic": 0.0}),
    # # меняется только mixup
    dict(name="E09_mixup_0p2", model=BASE_MODEL, override={"mixup": 0.2}),
    # # меняется только close_mosaic
    dict(name="E10_close_mosaic_10", model=BASE_MODEL, override={"close_mosaic": 10}),
]
...
def append_summary(row: dict) -> None:
    # гарантируем стабильный набор колонок (в первой строке задаём заголовок)
...
def main() -> None:
    ...
    # фикс dataset.yaml под локальный путь (убираем Windows/Colab остатки)
